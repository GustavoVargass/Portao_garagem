...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        _, possiblePlate.imgThresh = cv2.threshold(possiblePlate.imgThresh, 0.0, 255.0, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        oldImgThresh = possiblePlate.imgThresh.copy()
        possiblePlate.imgThresh = CharacterImage.getCharactersFromImage(possiblePlate.imgThresh)
        try:
            findPossibleCharsInPlate(possiblePlate.imgGrayscale, possiblePlate.imgThresh)
        except:
            possiblePlate.imgThresh = oldImgThresh
